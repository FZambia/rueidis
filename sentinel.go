...
func newSentinelClient(opt *ClientOption, connFn connFn, retryer retryHandler) (client *sentinelClient, err error) {
	...
	// Opt-in. Off by default keeps this change self-contained; see
	// SentinelOption.TopologyRefreshInterval for why enabling it is strongly
	// recommended, and why the sentinel client has no other recovery path.
